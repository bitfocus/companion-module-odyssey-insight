...
export class ModuleInstance extends InstanceBase<ModuleConfig> {
	...
	async init(config: ModuleConfig): Promise<void> {
		...
		await this.initializeStatusUpdate()
	}
	...
	async requestCommand(commandName: string): Promise<any> {
		const url = this.getInsightChannelCommand(commandName)
		const promise = axios
			.post(url, undefined, {
				headers: {
	...
	sendCommand(commandName: string): void {
		try {
			this.requestCommand(commandName).catch((_rejected) => {
				this.log('error', 'Send request failed.')
			})
		} catch (e) {
	...
	async initializeStatusUpdate(): Promise<void> {
		this.statusUpdateHandle = setInterval(() => {
			;(async () => {
				...
				this.log('error', 'Error executing interval handler.')
			})
		}, this.config.refreshPeriod)
	}

	async doStatusUpdate(): Promise<void> {
		return this.requestCommand('Status').then((response) => {
			response = response as Promise<AxiosResponse<any, any, unknown>>
